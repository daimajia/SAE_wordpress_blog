	...
	if(!is_user_logged_in()){
		...
		if(smc_is_administrator()){
			...
			if($_GET['page']=='social-medias-connect/function.php' && is_admin() && isset($r)){
				if(is_array($r)){
					$tok=array('user_login'=>$r['user_login'],'weibo'=>$r['weibo'],'uid'=>get_current_user_id(),'oauth_token'=>$r['oauth_access_token'],'oauth_token_secret'=>$r['oauth_access_token_secret']);
...
if(!function_exists('connect_login_form_login')){
	...
	function connect_login_form_login(){
		if(is_user_logged_in()){
			...
			$redirect_to.='?oauth_token=smc_weibo&code=smc_facebook';
...
function smc_bind_weibo_uninstall(){
	require_once 'uninstall-menu.php';
}
...
function smc_is_administrator(){
	...
	if($user->data->wp_user_level >=10 || $user->data->user_level >=10)$is_administrator=true;
