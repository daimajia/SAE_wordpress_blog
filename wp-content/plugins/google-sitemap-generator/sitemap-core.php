...
class GoogleSitemapGenerator {
	...
	*/
	function GetXmlUrl($forceAuto=false) {
		...
		$stor = new SaeStorage();
		return $stor->getUrl('wordpress', $this->GetOption("b_filename"));
	...
	*/
	function GetXmlPath($forceAuto=false) {
		return "saestor://wordpress/" . $this->GetOption("b_filename");
		if(!$forceAuto && $this->GetOption("b_location_mode")=="manual") {
	...
	function BuildSitemap() {
		...
		if($this->IsGzipEnabled()) {
			...
			if($this->IsFileWritable($fileName)) {
				...
				$gzfile = tempnam(NULL, 'SITEMAP');
				$this->_fileZipHandle = gzopen($gzfile,"w1");
...
//			if($pingres==NULL || $pingres===false || strpos($pingres," ")===false) {
			if($pingres==NULL || $pingres===false || strpos($pingres,"感谢您提交站点地图")===false) {
